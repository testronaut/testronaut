...
- **`inPageWithFunctionName`** - A last resort for rare scenarios requiring explicit function naming
...
## `inPageWithFunctionName` (Last Resort)

**Warning: This should only be used as a last resort.**

The `inPageWithFunctionName` function requires an explicit function name as the first argument. This name serves as a unique identifier for the function.

### When to Use

Use `inPageWithFunctionName` only when you need to perform **multiple distinct browser actions within the same test file** that need to be uniquely identified at runtime.
...
```ts
test('example', async ({ page, inPageWithFunctionName }) => {
  await inPageWithFunctionName('setup-dom', () => {
    document.body.textContent = 'Hello!';
  });
...
```ts
test('with data', async ({ page, inPageWithFunctionName }) => {
  await inPageWithFunctionName('greet-user', { name: 'World' }, ({ name }) => {
    document.body.textContent = `Hello ${name}!`;
  });
...
The named `inPageWithFunctionName` variant exists for backward compatibility and edge cases where explicit naming is unavoidable. In most tests, you should never need it.
...
```ts
// New API
await inPageWithFunctionName('my-function', () => { ... });
```
